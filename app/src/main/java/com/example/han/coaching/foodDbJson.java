...
public class foodDbJson extends AsyncTask<String,Void,Void> {
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        try{
            ...
            if(!(staticMerge.foodAnni == null)) {
                Searcher searcher1 = new Searcher();
                searcher1.searchKeyword(MainActivity.mContext, staticMerge.foodAnni.get(0), latitude, longitude, radius, page, 2, "9db6272582177f1d7b0643e35e1993e9", new OnFinishSearchListener() {
                    @Override
    ...
    protected Void doInBackground(String... params) {
        ...
        if(Annis == null){
            ...
            staticMerge.finish_food[3] = Annis.get(0).getCate();
        }
        if(staticMerge.finish_food[3].equals("empty")){
            staticMerge.foodAnni = null;
        }else{
            try {

                URL url = new URL("http://222.116.135.76:8080/Noon/createJson.jsp?" + "tbName=noon_food&col=food_name&food_wea="+staticMerge.temp+"&food_name="+staticMerge.finish_food[3]+";");
                Log.i("aaaa", "--------------------33333333333211111111111111111111111" + "http://222.116.135.76:8080/Noon/createJson.jsp?" + "tbName=noon_food&col=food_name&food_wea="+staticMerge.temp+"&food_name="+staticMerge.finish_food[3]+";");
                URLConnection conn = (HttpURLConnection) url.openConnection();
                HttpURLConnection http = (HttpURLConnection) conn;
